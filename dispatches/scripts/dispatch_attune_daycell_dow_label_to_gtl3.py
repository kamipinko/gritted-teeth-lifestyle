...
prompt = "After you finish the calendar height fix (dispatches/attune_calendar_height_fix.md), do this next: read dispatches/attune_daycell_dow_label.md and execute. /attune cells currently render only the muscle kanji — no day-of-week, no day number, no English pairing. Edit only components/attune/DayCell.jsx: (1) add MUSCLE_LABEL map (chest→CHEST etc.) next to MUSCLE_KANJI, (2) compute dowLabel + dayNum from dayId via local-tz Date, (3) restructure cell to render top header (DOW left + day number right, with ★ inline before DOW for source day), kanji centered (smaller at far tier), English label below kanji, move chip-count badge to bottom-left, bump far minHeight 84→100. Don't touch CycleCalendar.jsx or page.js or anything else. Screenshot /attune at 390x844 with a populated 7-day cycle showing all 11 muscles to verify the kanji↔English pairing. Commit + push origin/dev as a SECOND commit (separate from the calendar height fix). Report SHA + screenshot."
