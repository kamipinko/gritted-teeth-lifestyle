...
COMMANDS:
  ...
  record [WindowTitle] [fps] [dur]  Record motion — contact sheet of frames
                                    fps default=2, dur default=4s, cols=4
                                    e.g. record "Chrome" 3 6
...
# ── Record (motion contact sheet) ─────────────────────────────────────────────

def record(window_title=None, fps=2, duration=4, cols=4):
    """
    Capture frames at `fps` for `duration` seconds, stitch into a contact
    sheet grid so Claude can see the full motion in one image.
    ...
    # Resolve capture bbox once
    bbox = None
    win_label = "full screen"
    if window_title:
        matches = gw.getWindowsWithTitle(window_title)
        if not matches:
            print(f"ERROR: No window matching '{window_title}'")
            sys.exit(1)
        win = matches[0]
        try:
            win.activate()
    ...
    print(f"Recording '{win_label}'  {fps}fps x {duration}s = {total_frames} frames ...")
    ...
    for i in range(total_frames):
        ...
        img = ImageGrab.grab(bbox=bbox)
        img = _normalize(img)   # normalize each frame individually
        frames.append(img)
        timestamps.append(time.time() - start)
        print(f"  frame {i+1}/{total_frames}  t={timestamps[-1]:.2f}s  size={img.size}")
    ...
    # Build contact sheet
    pad = 4
    # Scale each frame to thumbnail size so the sheet stays readable (~1800px wide)
    thumb_w = min(frames[0].size[0], (1800 - pad * (cols + 1)) // cols)
    ratio = thumb_w / frames[0].size[0]
    thumb_h = int(frames[0].size[1] * ratio)
    frames = [f.resize((thumb_w, thumb_h), Image.LANCZOS) for f in frames]

    fw, fh = frames[0].size
    rows = (len(frames) + cols - 1) // cols
    label_h = 20
    sheet_w = cols * (fw + pad) + pad
    sheet_h = rows * (fh + label_h + pad) + pad
    ...
    for idx, (frame, ts) in enumerate(zip(frames, timestamps)):
        ...
        row = idx // cols
        x = pad + col * (fw + pad)
        y = pad + row * (fh + label_h + pad)
        sheet.paste(frame, (x, y))
        label = f"t={ts:.2f}s"
        draw.rectangle([x, y + fh, x + fw, y + fh + label_h], fill=(30, 30, 30))
        draw.text((x + 4, y + fh + 2), label, fill=(200, 200, 200), font=font)

    sheet.save(SCREENSHOT_PATH)
    size_kb = os.path.getsize(SCREENSHOT_PATH) // 1024
    print(f"Contact sheet: {cols}x{rows} grid  {sheet_w}x{sheet_h}px  {size_kb}KB")
...
def main():
    ...
    if cmd == "screenshot":
        ...
        # record [WindowTitle] [fps] [duration] [cols]
        # Args are positional but WindowTitle can be multi-word, so detect numbers
        rest = sys.argv[2:]
        # Pull trailing numeric args off the end
        cols = 4
        duration = 4.0
        fps = 2.0
        title_parts = []
        ...
        if len(num_args) >= 1: fps = num_args[0]
        if len(num_args) >= 2: duration = num_args[1]
        if len(num_args) >= 3: cols = int(num_args[2])
        title = " ".join(title_parts) if title_parts else None
        record(title, fps=fps, duration=duration, cols=cols)
